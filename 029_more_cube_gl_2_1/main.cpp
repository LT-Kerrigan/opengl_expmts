...
This is an OpenGL 2.1 compatible version of the cube-map omni-directional shadow
projection / sampler

* 2d depth texture works instead of 3d cube depth texture in newer GL
* renderbuffer works instead of 2d depth texture
* converting shaders to GLSL 120 works just fine
* gl_FragColor is limited to vec4 so (l,l,l,1.0) is fine
* glTexImage2D() has MUCH more restrictive formatting options in GL 2.1
  * must use all 3 or 4 channels
  * can't be float format, can be integers and shorts though
  * can be fancy combos of rgba in different sizes up to max combined 32-bits
Notes:
...
// create a renderbuffer which allows depth-testing in the framebuffer 
	printf ("creating renderbuffer\n");
  GLuint rb = 0;
  glGenRenderbuffers (1, &rb);
  glBindRenderbuffer (GL_RENDERBUFFER, rb);
  glRenderbufferStorage (GL_RENDERBUFFER, GL_DEPTH_COMPONENT, g_cube_map_dims,
  	g_cube_map_dims);
  ...
  glFramebufferRenderbuffer (GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
	...
	glTexImage2D (GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, g_cube_map_dims, g_cube_map_dims, 0,
